...
class GenomeBase:
   ...
   def evaluate(self, **args):
      ...
      for it in self.evaluator.applyFunctions(self, **args):
         if self.evaluator.funcList[i].__name__ == 'eval_func_fscore' :
            self.fscore = it[0]
            self.accuracy = it[1]
         elif self.evaluator.funcList[i].__name__ == 'eval_func_hamdist':
            hamdist = it
         elif self.evaluator.funcList[i].__name__ == 'eval_func_eucdist':
            eucdist = it
         elif self.evaluator.funcList[i].__name__ == 'eval_func_entropy':
            intropy = it
         elif self.evaluator.funcList[i].__name__ == 'eval_func_information_gain':
            information_gain = it
         i = i + 1

      self.hamdist = hamdist
      self.eucdist = eucdist
      self.information_gain = information_gain
      self.intropy = intropy
      self.fitness = self.fscore * self.hamdist * information_gain * intropy * eucdist
   ...
   def copy(self, g):
      ...
      g.fscore = self.fscore
      g.hamdist = self.hamdist
      g.eucdist = self.eucdist
      g.information_gain = self.information_gain
      g.intropy = self.intropy
      g.accuracy = self.accuracy
...
class GTreeNodeBase:
   ...
   def __init__(self, parent, childs=None):
      ...
      if childs is not None:
         if type(childs) != list:
            gp.Util.raiseException("Childs must be a list of nodes", TypeError)
   ...
   def addChild(self, child):
      ...
      if type(child) == list:
         ...
         if not isinstance(child, GTreeNodeBase):
            gp.Util.raiseException("The child must be a node", TypeError)
...
class GTreeBase:
   ...
   def __init__(self, root_node):
      ...
      # selfdefined variables
      self.ecoc_matrix = None
      self.feature_usedList = None
      self.nodes_leaf = None
      self.nodes_branch = None
   ...
   def setRoot(self, root):
      ...
      if not isinstance(root, GTreeNodeBase):
         gp.Util.raiseException("The root must be a node", TypeError)
      self.root_node = root
   ...
   def traversal(self, callback, start_node=None):
      ...
      if not inspect.isfunction(callback):
         gp.Util.raiseException("The callback for the tree traversal must be a function", TypeError)
   ...
   def getLeafs(self):
      return self.nodes_leaf

   def convertTree(self):
       import ecoc.TreeMatrixConvertor  as tmc
       self.ecoc_matrix, self.feature_usedList = tmc.getMatrixDirectly_and_feature(self)

   def getMatrix(self):
      return self.ecoc_matrix

   def getFeatureList(self):
      return self.feature_usedList